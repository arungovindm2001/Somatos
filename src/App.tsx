...
import correctPosture from "./sounds/posture.mp3";
import unidentifiedUser from "./sounds/user.mp3";
import "./App.css";
...
function App() {
	...
	const [posture] = useSound(correctPosture, { volume: 0.5 });
	const [unknownUser] = useSound(unidentifiedUser, { volume: 0.5 });
	...
	const displayErrorToast = (message: string) => {
		if (message === "Unable to detect user.") {
			unknownUser();
			toast({
				position: "bottom-left",
		...
		else if (message === "Your posture requires correction!") {
			posture();
			toast({
				position: "bottom-left",
				title: "An error occurred.",
